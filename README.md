...
- **Trail lighting preview** — a widget in the bottom-right corner shows a real photo of the trail (left) with lighting matched to the sun's elevation (Boquete, Panama, April 1). Seven pre-graded keyframes (day → golden hour → sunset → twilight → night, generated by `make_keyframes.py` with tone curves and Purkinje blue shift) are cross-faded for realistic transitions. On the right: a schematic horizon diagram with elevation arcs (30°, 60°), the sun position at current elevation/azimuth, and a mountain silhouette at 1500 m. Updates every 5 simulated minutes. Hideable via the toggle button above the widget.
